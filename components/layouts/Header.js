		...
		<div>
			...
			<nav className={active ? 'active' : ''}>
				...
				<div className='social'>
					{navItems.map((item, index) => (
						<div className='row menu-item'>
							<a href={item.link} key={index} alt={item.alt}>
								<Icon name={item.name} />
							</a>
						</div>
					))}
					<a href='https://vsco.co/brrian' className='icon icon-vsco' alt='VSCO'>
						<svg
							width='22'
							height='22'
							viewBox='0 0 24 24'
				...
				<List>
					...
					<ListItem name='Aafdsfsaaaa' />
					<ListItem name='Aaadsfsdfaaa' />
					<ListItem name='Aaadsfsdfdsfaaa' />
				</List>
			</nav>
		</div>
	)
